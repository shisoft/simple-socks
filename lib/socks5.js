...
function SocksServer (options) {
	...
	self.server = net.createServer(function (socket) {
				...
				.tap(function (args) {
					...
					self.options.authenticate(
						...
						authDomain.intercept(function () {
							...
							socketUname = args.uname.toString();
							self.server.emit(EVENTS.AUTHENTICATION, socketUname);
		...
		function end (response, args) {
			...
			self.server.emit(EVENTS.PROXY_END, socketUname, response, args);
		}
		...
		socket.once('end', function () {
			...
			self.server.emit(EVENTS.SOCKS_END, socketUname, socket);
		});
	});
